...
truncate input_embedding_view; --add index on token_id, col_tile
truncate query_cache; 
truncate key_cache;
truncate value_cache;
...
LEFT JOIN attention_norm AS df_ffn_norm ON raw_embedding_view.col_tile = df_ffn_norm.col_tile
WHERE df_ffn_norm.layer_id = 0);


insert or replace into query_cache select * from (
with query_partial_results AS (
        SELECT
            token_id,
            B.head_id,
        ...
        FROM
            WQ AS B
            join input_embedding_view as A ON A.col_tile = B.col_tile
        WHERE
            layer_id = 0
    ),
...
with triu AS (
    ...
    FROM query_cache AS Q
    JOIN key_cache kd 
      ON Q.token_id >= kd.token_id 
     AND kd.head_id  = floor(Q.head_id / 4)
),
attention_score as (select head_id,q_token_id,k_token_id, exp(value-60)::Float as score from triu),
exp_sums as (select head_id,q_token_id,sum(score)::Float as summation from attention_score group by head_id,q_token_id),
softmax_score as (select A.head_id,A.q_token_id,A.k_token_id,score/summation as softmax_score from attention_score as A left join exp_sums as B on A.head_id=B.head_id and A.q_token_id=B.q_token_id ),
partial_score AS (
    SELECT
        qk.head_id,
        qk.q_token_id,
...
embedding_delta as (select q_token_id,groups as col_tile,collect_as_array(list(row_tile),list(value))::Float[] as delta from delta_value group by q_token_id,groups)
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=0 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=0 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=1 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=1 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=2 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=2 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=3 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=3 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=4 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=4 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=5 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=5 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=6 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=6 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=7 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=7 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=8 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=8 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=9 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=9 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=10 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=10 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=11 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=11 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=12 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=12 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=13 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=13 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=14 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=14 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=15 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=15 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=16 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=16 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=17 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=17 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=18 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=18 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=19 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=19 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=20 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=20 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=21 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=21 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=22 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=22 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=23 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=23 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=24 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=24 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=25 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=25 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=26 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=26 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=27 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=27 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=28 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=28 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=29 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=29 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=30 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=30 group by token_id,row_tile ),
swish as (
...
with W1_agg as(select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r1 
from W1 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=31 group by token_id,row_tile),
W3_agg as (select token_id,B.row_tile as row_tile, sum(list_dot_product(embedding,chunk)) as r3 
from W3 as B join embedding_after_norm as A on A.col_tile=B.col_tile 
where layer_id=31 group by token_id,row_tile ),
swish as (
